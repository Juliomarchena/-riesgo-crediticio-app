        ...
        children: [
          ...
          new Table({
            ...
            rows: [
              ...
              makeRow("Probabilidad de Incumplimiento", result.probabilidad_riesgo ? `${(result.probabilidad_riesgo * 100).toFixed(1)}%` : "-", true),
              makeRow("Probabilidad de Cumplimiento", result.probabilidad_no_riesgo ? `${(result.probabilidad_no_riesgo * 100).toFixed(1)}%` : "-", false),
          ...
          new Table({
            ...
            rows: [
              ...
              makeRow("Politica Crediticia", result.datos_recibidos?.credit_policy === 1 ? "Cumple Criterios" : "No Cumple", false),
              ...
              makeRow("Cuota Mensual", result.datos_recibidos?.installment ? `$${result.datos_recibidos.installment}` : "-", true),
              makeRow("Log Ingreso Anual", result.datos_recibidos?.log_annual_inc ?? "-", false),
              makeRow("Puntaje FICO", result.datos_recibidos?.fico ?? "-", true),
              makeRow("Ratio Deuda/Ingreso (DTI)", result.datos_recibidos?.dti ?? "-", false),
              makeRow("Dias con Linea de Credito", result.datos_recibidos?.days_with_cr_line ?? "-", true),
              makeRow("Balance Revolving", result.datos_recibidos?.revol_bal ? `$${result.datos_recibidos.revol_bal}` : "-", false),
              makeRow("Utilizacion Revolving", result.datos_recibidos?.revol_util ? `${result.datos_recibidos.revol_util}%` : "-", true),
              makeRow("Consultas ultimos 6 meses", result.datos_recibidos?.inq_last_6mths ?? "-", false),
              makeRow("Morosidades (2 anos)", result.datos_recibidos?.delinq_2yrs ?? "-", true),
              makeRow("Registros Publicos Negativos", result.datos_recibidos?.pub_rec ?? "-", false),
            ],
          }),
